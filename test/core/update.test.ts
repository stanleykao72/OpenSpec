...
`
    ...
    it('should update core profile skill files when tool is configured', async () => {
      ...
      // Verify core profile skill files were created/updated (propose, explore, apply, update, verify, sync, archive)
  ...
  describe('command updates', () => {
    ...
    it('should update core profile opsx commands when tool is configured', async () => {
      ...
      // Verify core profile commands were created (propose, explore, apply, update, verify, sync, archive)
      const coreCommandIds = ['explore', 'apply', 'update', 'verify', 'sync', 'archive', 'propose'];
      const commandsDir = path.join(testDir, '.claude', 'commands', 'opsx');
      ...
      const nonCoreCommandIds = ['new', 'continue', 'ff', 'bulk-archive'];
  ...
  describe('profile-aware updates', () => {
    ...
    it.each(['skills', 'commands', 'both'] as const)(
      ...
      async (delivery) => {
        setMockConfig({
          ...
          profile: 'custom',
          delivery,
          // 'verify' is part of the fork's CORE_WORKFLOWS; without it these
          // fixtures report two missing core workflows instead of one.
    ...
    it('should list a single missing core workflow when custom profile lacks only update', async () => {
      setMockConfig({
        ...
        profile: 'custom',
        delivery: 'both',
        workflows: ['propose', 'explore', 'apply', 'verify', 'sync', 'archive'],
      });
...
`
    ...
    it('should remove workflows outside profile during update sync', async () => {
      // Set core profile (propose, explore, apply, verify, sync, archive)
      setMockConfig({
        featureFlags: {},
        profile: 'core',
        delivery: 'both',
