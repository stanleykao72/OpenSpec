...
export function getTransformerForTool(
  ...
  if (delivery === 'skills' || capability !== 'adapter-backed') {
    // Fork: `codex` (upstream, since v1.7.0) and `grok` (fork) both write their
    // skills into the SHARED `.agents/skills/` tree, so whichever runs last owns
    // the file. The codex-compatible form emits BOTH `$openspec-*` and
    // `/openspec-*`, so it is a superset that keeps codex working when grok
    // writes last, and costs grok nothing (it still sees `/openspec-*`).
    return toolId === 'codex' || toolId === 'grok'
