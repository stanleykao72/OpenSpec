...
export async function buildUpdatedSpec(
  ...
  try {
    ...
    isNewSpec = true;
    // Upstream's Purpose carry-over is judged on the frontmatter-free skeleton
    // (the readability checks parse markdown structure); the fork's frontmatter
    // is added afterwards so both features hold at once.
    const baseSkeleton = buildSpecSkeleton(specName, changeName, deltaPurpose);
    const overview = deltaPurpose ? readableOverview(baseSkeleton, specName) : null;
    let carriedPurpose = deltaPurpose;
    ...
    targetContent = buildSpecSkeleton(specName, changeName, carriedPurpose, {
      archivedOn,
  ...
  const assembled = [parts.before.trimEnd(), parts.headerLine, reqBody, parts.after.trim()]
    .filter((s) => s !== '')
    .join('\n\n')
    .replace(/\n{3,}/g, '\n\n')
    .trimEnd() + '\n';
...
function contentTheMergeCannotName(parts: RequirementsSectionParts): string[] {
  ...
  // A leading frontmatter block IS content the merge can name: `buildSpecSkeleton`
  // writes it and `appendArchiveSource` maintains its `sources:` list. Leaving it
  // in would report every fork-authored spec as unaccounted-for and refuse every
  // retirement. An unterminated block still parses as ordinary content, so the
  // conservative "refuse rather than delete" default is unchanged.
  const beforeWithoutFrontmatter = parseLeadingFrontmatter(
    parts.before.replace(/^\uFEFF/, '')
  ).body;
  const beforeLines = beforeWithoutFrontmatter.replace(/\r\n?/g, '\n').split('\n');
...
export function buildSpecSkeleton(
  ...
  // Fork wording: the upstream placeholder read like a filled-in field and
  // survived into merged specs. Keep it obviously a to-do. Upstream's carry-over
  // still wins when the delta authored its own Purpose.
  const purposeBody =
    purpose?.trim() ||
    `TBD(archive): describe what this capability is for and replace this line ` +
      `— created by archiving change ${changeName}.`;

  // Fork customization: emit the frontmatter the archive workflow documents
  // (`type: capability`, `id`, `sources`) so a freshly created capability spec
  // is not left for a human to hand-patch. `module` / `scope` are carried over
  // from the delta spec when it declares them and omitted otherwise - the
  // skeleton never invents a value it was not given.
  const archivedOn = options.archivedOn ?? archiveDateStamp();
