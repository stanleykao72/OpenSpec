...
export class ArchiveCommand {
  ...
  private async run(
    ...
    // ENOTEMPTY from rename would otherwise degrade to a bare `archive_error`.
      ...
      // Fork plugin/gate system: archive.post hooks are side effects only —
      // upstream's ArchiveResult remains the return contract so --json output
      // stays pure and callers see the same shape.
      if (plugins.length > 0) {
